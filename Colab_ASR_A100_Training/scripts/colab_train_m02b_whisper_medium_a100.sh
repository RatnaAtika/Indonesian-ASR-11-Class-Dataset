...
echo "[medium-a100] RUN_DIR=$RUN_DIR batch=$BATCH grad_accum=$ACCUM effective=$((BATCH*ACCUM)) workers=$NUM_WORKERS test_batch=$TEST_BATCH sync_interval=${SYNC_INTERVAL_SEC}s"
...
if [[ "${A100_CONSOLE_LOG:-0}" == "1" ]]; then
  python3 training/m02b_whisper_medium_ft/test.py --run-dir "$RUN_DIR" --data-root "$DATA_ROOT" --data-final "$DATA_FINAL" --batch-size "$TEST_BATCH" \
    2>&1 | tee "$TEST_LOG"
else
  python3 training/m02b_whisper_medium_ft/test.py --run-dir "$RUN_DIR" --data-root "$DATA_ROOT" --data-final "$DATA_FINAL" --batch-size "$TEST_BATCH" \
