# Value of the Data — Indonesian limited-data ASR corpus (retake2026)
...
- The corpus enables **fair benchmarking across 20 native speakers**
  (10 F / 10 M) and **11 sentence-type categories** under uniform
...
- The shipped train / dev / test split is **speaker-disjoint with zero
  ...
  to re-derive a leakage-safe partition.
...
- The synthetic-data fraction is **explicitly disclosed at every level**
  ...
  set), enabling reuse for studies that need to control for
  TTS-contamination effects.

- A stratified audio-quality sample (n = 297 files, one ≈ 27 per
  category) is shipped alongside the raw audio, giving a calibrated
  baseline for acoustic-quality audits and feature-extraction
  pipelines.
