...
Final table source CSV/DOCX files and figure source CSV/script files will be generated under [`Draft_Paper/04_Revised_Draft/`](.) and copied to [`Draft_Paper/05_Submission_Package/`](../05_Submission_Package) only after scope, privacy, and checksum verification. Existing figures are evidence inputs, not automatically approved submission assets.
