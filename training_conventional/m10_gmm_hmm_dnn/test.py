"""m10_gmm_hmm_dnn test entry point — paper model (mode=gmm_hmm_dnn)."""
...
if "--run-dir" not in extra:
    ...
    if cands:
        ...
        print(f"[m10_gmm_hmm_dnn-test] ERROR: no run found"); sys.exit(1)
cmd = ["python3", str(TC / "common/pkl_hmm_test.py"),
       "--mode", "gmm_hmm_dnn"] + extra
print(f"[m10_gmm_hmm_dnn-test] cmd: {' '.join(cmd)}")
