...
from transformer_model_vit import TransformerASR
...
class ASRDataset(Dataset):
    def __init__(self, data, specaug: bool = False):
        self.X = data['X']
        self.y = data['y']
    ...
    def __getitem__(self, idx):
        ...
        if self.specaug and torch.rand(1).item() < 0.8:
...
# ---- SpecAugment (light) ----
def apply_specaugment(x: torch.Tensor,
                      freq_mask_param: int = 15,
                      time_mask_param: int = 30,
                      num_masks: int = 2) -> torch.Tensor:
    # x: [T, F]
    T, F = x.size(0), x.size(1)
    x_aug = x.clone()
...
def train():
    ...
    parser.add_argument('--outdir', default='runs/vit', help='Output directory for plots and last checkpoint')
    ...
    parser.add_argument('--checkpoint', default='checkpoints/best-transformer-asr-vit.pth')
    parser.add_argument('--lambda-ctc', type=float, default=0.0, help='aux CTC weight (0.0 = disabled)')
    parser.add_argument('--scheduler', type=str, default='plateau', choices=['plateau','noam','cosine'], help='LR scheduler type')
    ...
    print(f"[Config] lambda_ctc={args.lambda_ctc}")
    print(f"[Config] scheduler={args.scheduler} warmup_steps={args.warmup_steps}")
    ...
    # test set optional
    train_set = ASRDataset(train_data, specaug=args.specaug)
    val_set = ASRDataset(val_data, specaug=False)
    ...
    try:
        ...
        try:
            ...
            plt.savefig(os.path.join(args.outdir, 'model_summary_vit.png'), bbox_inches='tight')
            plt.savefig(os.path.join(args.outdir, 'model_summary_vit.pdf'), bbox_inches='tight')
            plt.close(fig)
    ...
    if args.scheduler == 'plateau':
        ...
        scheduler_step = lambda metric: None
    else:  # cosine
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(1, args.epochs))
        scheduler_step = lambda metric: scheduler.step()

    ctc_loss_fn = None
    if args.lambda_ctc > 0.0:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, (x, src_key_padding_mask, y_in, y_out) in enumerate(train_iter):
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                if ctc_loss_fn is None:
                    ...
                    logits, enc_mem, enc_pad = model.forward_with_memory(x, y_in, src_key_padding_mask=src_key_padding_mask, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask)
                    ce = criterion(logits.view(-1, logits.size(-1)), y_out.view(-1))
                    # CTC branch
                    with torch.no_grad():
                        input_lengths = (~enc_pad).sum(dim=1).to(torch.long)  # [B]
                        # prepare targets: remove BOS and EOS and PAD(0)
                        ys_ctc = []
                        target_lengths = []
                        for seq in y_out:
                            seq_ids = [int(t) for t in seq.tolist() if int(t) != 0 and int(t) != 2 and int(t) != 3]
                            target_lengths.append(len(seq_ids))
                            ys_ctc.extend(seq_ids)
                        target_lengths = torch.tensor(target_lengths, dtype=torch.long, device=device)
                        if len(ys_ctc) == 0:
                            ys_ctc = [1]
                        targets = torch.tensor(ys_ctc, dtype=torch.long, device=device)
                    logp = torch.log_softmax(model.ctc_head(enc_mem), dim=-1)  # [B,T_enc,V]
                    logp = logp.transpose(0, 1)  # [T_enc,B,V]
                    try:
                        ctc = ctc_loss_fn(logp, targets, input_lengths, target_lengths)
                    except Exception:
                        ctc = torch.tensor(0.0, device=device)
                    if 0.0 <= args.lambda_ctc <= 1.0:
                        loss = (1.0 - args.lambda_ctc) * ce + args.lambda_ctc * ctc
                    else:
                        loss = ce + args.lambda_ctc * ctc
            ...
            if ctc_loss_fn is None:
                total_train_ce += float(loss.item())
            else:
                total_train_ce += float(ce.item())
                total_train_ctc += float(ctc.item())
        ...
        avg_train_ce = total_train_ce / max(1, len(train_loader))
        avg_train_ctc = total_train_ctc / max(1, len(train_loader)) if ctc_loss_fn is not None else 0.0
        ...
        if ctc_loss_fn is not None:
            print(f"[Train] avg_ce={avg_train_ce:.4f} avg_ctc={avg_train_ctc:.4f} (lambda_ctc={args.lambda_ctc})")
        # --- Validation ---
        model.eval()
        total_val_loss = 0
        total_val_cer = 0
        ...
        for pred, label in zip(val_pred_strs[:5], val_label_strs[:5]):
            print(f"PRED: {pred}\nLABEL: {label}\n")
    torch.save(model.state_dict(), os.path.join(args.outdir, 'transformer_asr_vit_last.pth'))
    end_time = time.time()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training & Validation Loss (VIT)')
    plt.savefig(os.path.join(args.outdir, 'training_val_loss_vit.png'))
    ...
    plt.title('Training & Validation Char Accuracy (VIT)')
    plt.savefig(os.path.join(args.outdir, 'training_val_accuracy_vit.png'))
    plt.ylabel('Char Accuracy')
    plt.title('Training & Validation Character Accuracy (VIT)')
    plt.savefig(os.path.join(args.outdir, 'char_accuracy_vit.png'))
    ...
    plt.xlabel('Epoch')
    plt.ylabel('CER')
    plt.title('Training & Validation CER (VIT)')
    plt.savefig(os.path.join(args.outdir, 'cer_vit.png'))
